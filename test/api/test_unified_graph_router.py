...
async def test_graph_routes_require_auth(test_client):
    ...
    list_response = await test_client.get("/api/graph/list")
    assert list_response.status_code == 401

    info_response = await test_client.get("/api/graph/neo4j/info")
...
async def test_standard_user_cannot_access_graph_endpoints(test_client, standard_user):
    ...
    list_response = await test_client.get("/api/graph/list", headers=standard_user["headers"])
    assert list_response.status_code == 403

    info_response = await test_client.get("/api/graph/neo4j/info", headers=standard_user["headers"])
...
async def test_get_graphs_list(test_client, admin_headers):
    ...
    # The default graph is exposed through the upload adapter facade.
    assert neo4j_graph["type"] == "upload"
    assert "capabilities" in neo4j_graph

    # Note: LightRAG graphs might be empty if none created, but we check structure


async def test_get_neo4j_info_requires_admin(test_client, standard_user, admin_headers):
    """Test that Neo4j info follows the same auth boundary as graph listing."""
    user_response = await test_client.get("/api/graph/neo4j/info", headers=standard_user["headers"])
    assert user_response.status_code == 403

    admin_response = await test_client.get("/api/graph/neo4j/info", headers=admin_headers)
