...
BLUEOCEAN_CHUNK_DELIM_ENV = "BLUEOCEAN_CHUNK_DELIM"
LEGACY_CHUNK_DELIM_ENV = f"{_LEGACY_BRAND_PREFIX}_CHUNK_DELIM"
DEFAULT_BLUEOCEAN_CHUNK_DELIM = "\n<|BLUEOCEAN_CHUNK_DELIM|>\n"
_chunk_delim_deprecation_warned = False


def _resolve_chunk_delimiter() -> str:
    """Read canonical delimiter first and keep one-round backward compatibility."""
    delimiter = os.getenv(BLUEOCEAN_CHUNK_DELIM_ENV)
    if delimiter:
        return delimiter

    legacy_delimiter = os.getenv(LEGACY_CHUNK_DELIM_ENV)
    if legacy_delimiter:
        global _chunk_delim_deprecation_warned
        if not _chunk_delim_deprecation_warned:
            logger.warning(
                "Legacy chunk delimiter environment variable is deprecated; "
                "use BLUEOCEAN_CHUNK_DELIM instead."
            )
            _chunk_delim_deprecation_warned = True
        return legacy_delimiter

    return DEFAULT_BLUEOCEAN_CHUNK_DELIM
...
class LightRagKB(KnowledgeBase):
    ...
    def _prepare_lightrag_insert_payload(chunks: list[dict]) -> tuple[str, str | None, bool]:
        ...
        delimiter = _resolve_chunk_delimiter()
