    ...
    try:
        ...
        async for msg, metadata in agent.stream_messages(messages, input_context=input_context):
            if isinstance(msg, AIMessageChunk):
                chunk_content = msg.content or getattr(msg, "reasoning_content", None)
                if not chunk_content and isinstance(getattr(msg, "additional_kwargs", None), dict):
                    chunk_content = msg.additional_kwargs.get("reasoning_content")
                ...
                yield make_chunk(content=chunk_content, msg=msg.model_dump(), metadata=metadata, status="loading")
            else:
