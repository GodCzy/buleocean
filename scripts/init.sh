...
# BlueOcean Initialization Script for Bash/Linux/macOS
# This script helps set up the local BlueOcean development environment.

set -e

echo "Initializing BlueOcean project..."
echo "================================"

if [ -f ".env" ]; then
    echo ".env file already exists. Skipping environment setup."
else
    echo ".env file not found. Starting environment setup."
    echo ""

    echo "SiliconFlow API Key required"
    echo "Get your API key from: https://cloud.siliconflow.cn/"
    while true; do
...
echo "This starts the local BlueOcean development stack."
