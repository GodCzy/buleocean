<template>
  <div class="login-view wl-shell" :class="{ 'has-alert': serverStatus === 'error' }">
    <!-- 服务状态提示 -->
    ...
    <!-- 顶部导航：品牌名称 & 操作按钮 -->
    <nav class="login-navbar wl-shell-topbar">
      <div class="navbar-content">
        <div class="brand-container wl-brand" @click="goHome" style="cursor: pointer">
          <img v-if="brandLogo" :src="brandLogo" alt="logo" class="brand-logo wl-brand-logo" />
          <h1 class="brand-text wl-brand-text">
    ...
    <!-- 页面底部：版权信息等 -->
    <footer class="page-footer wl-shell-footer">
      <div class="footer-links wl-nav-pills">
        <a class="wl-nav-pill" href="/themes" @click.prevent="goThemes">主题分区</a>
        <span class="divider">|</span>
        <a class="wl-nav-pill" v-if="docsUrl" :href="docsUrl" target="_blank" rel="noopener noreferrer"
          >文档中心</a
        >
      </div>
      <div class="copyright">
...
const brandName = computed(() => {
  ...
  const brandNameRaw = infoStore.branding?.name?.trim() || '蓝海智询'
...
const resolveRedirectPath = () => {
  ...
  const normalizedQueryRedirect =
    typeof queryRedirect === 'string' ? queryRedirect.trim() : ''
  if (normalizedQueryRedirect && normalizedQueryRedirect !== '/login') {
    return normalizedQueryRedirect
  }
...
onMounted(async () => {
  ...
  if (userStore.isLoggedIn) {
    const redirectPath = resolveRedirectPath()
    sessionStorage.removeItem('redirect')
    await router.push(redirectPath || '/agent')
    return
  }
